...
//!
//! Provides [`HttpServer`], which binds a TCP listener and serves the
//! compiled frontend as a single-page application.
...
impl HttpServer {
    ...
    /// serving requests.
    pub async fn run(&self) -> WebResult<()> {
        ...
        let listener =
            tokio::net::TcpListener::bind(&address).await.map_err(|source| WebError::Bind {
                address: address.clone(),
                source,
            })?;

        axum::serve(listener, app).await.map_err(WebError::Serve)?;
